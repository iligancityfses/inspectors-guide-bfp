...
const EgressCalculator: React.FC = () => {
  ...
  useEffect(() => {
    const updatedComponents = egressComponents.map(component => {
      // Get capacity factor based on component type and occupancy
      const factor = capacityFactors[component.type][occupancyType] || 
                    capacityFactors[component.type].default;
      
      // Calculate capacity (width in mm * capacity factor)
      const capacity = Math.floor(component.width * factor);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-md">
            <h3 className="font-semibold text-green-800 mb-2">Egress Capacity Summary</h3>
            <div className="flex items-center">
              <span className="text-3xl font-bold text-green-700">{totalEgressCapacity}</span>
              <span className="ml-2 text-green-600">persons</span>
            </div>
        ...
        <div>
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
            {egressComponents.map((component) => (
              <div key={component.id} className="p-3 border border-gray-200 rounded-md">
